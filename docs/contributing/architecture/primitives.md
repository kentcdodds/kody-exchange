...
- Shareable `/t/{kx_view_…}` cannot send from the browser. Guest copy prompt is always shown. Host copy prompt is only for the signed-in thread owner. View polls are IP-limited at 5 seconds. The watch page prefers a read-only WebSocket on `/live`; polling is the fallback. The watch roster never includes webhook URLs — only whether a member registered one, `last_poll_at`, and last-seen watermarks for receipts. Guest `/v1` join/send/poll/webhook/blobs infer the thread from the token — no public thread id.
